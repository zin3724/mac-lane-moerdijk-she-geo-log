...
For brevity, we now write $Q^P$ to be the functor $\widehat{\mathbf{C}/(-)}(P_{(-)},Q_{(-)})\times P$. To see that $Q^P$ is indeed a functor in $\mathbf{Sets}^{\mathbf{C}^{op}}$, note that if $(A\xra{f}B)\in\mathbf{C}$, then
